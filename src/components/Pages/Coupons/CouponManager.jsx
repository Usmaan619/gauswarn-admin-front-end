...
const CouponManager = () => {
    ...
    try {
      ...
      if (res) {
        if (Array.isArray(res)) {
          ...
          // Fallback for direct object response (if it is just the coupons object)
  ...
  const handleToggleActive = async (coupon) => {
    try {
      // updateData helper uses: `${API_BASE_URL}/${endpoint}/${id}`
      const res = await updateData("coupons", coupon.id, {
        is_active: coupon.is_active ? 0 : 1,
      });
      // updateData returns response, so we check res.data.success
      if (res.data?.success) {
        toastSuccess(
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this coupon?")) {
      try {
        const res = await deleteData("/coupons", id);
        // deleteData returns response.data
        if (res.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCoupon) {
        ...
        res = await postData("/coupons", payload);
      }

      // postData and updateData return response object
      if (res.data?.success) {
        toastSuccess(
