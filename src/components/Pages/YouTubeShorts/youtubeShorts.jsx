...
const YoutubeShortsUploader = () => {
  ...
  const loadShorts = async () => {
    const res = await getData("/shorts/all");
    if (res.success) {
  ...
  const saveShort = async () => {
    ...
    try {
      let res;
      if (editItem) {
        res = await postData(`/shorts/${editItem}`, payload);
      } else {
        res = await postData("/shorts", payload);
      }
  ...
  const deleteShort = async (id) => {
    if (!window.confirm("Are you sure you want to delete this YouTube Short?"))
      return;
    const res = await deleteDataNew(`/shorts-delete/${id}`);
    if (res.success) {
